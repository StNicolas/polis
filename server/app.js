...
helpersInitialized.then(
  (o) => {
    ...
    app.post('/api/v3/auth/pwresettoken', need('email', getEmail, assignToP), handle_POST_auth_pwresettoken);
    app.post(
    ...
    app.get('/api/v3/domainWhitelist', moveToBody, auth(assignToP), handle_GET_domainWhitelist);
    app.post(
    ...
    app.post('/api/v3/tutorial', auth(assignToP), need('step', getInt, assignToP), handle_POST_tutorial);
    app.get(
      '/api/v3/users',
      moveToBody,
      authOptional(assignToP),
    ...
    app.get('/api/v3/contexts', moveToBody, authOptional(assignToP), handle_GET_contexts);
    app.post(
      '/api/v3/contexts',
      auth(assignToP),
    ...
    app.post('/api/v3/reserve_conversation_id', auth(assignToP), handle_POST_reserve_conversation_id);
    app.post(
      '/api/v3/conversations',
      auth(assignToP),
    ...
    app.get('/api/v3/verify', moveToBody, need('e', getStringLimitLength(1, 1000), assignToP), handle_GET_verification);
    app.post(
      '/api/v3/contributors',
      authOptional(assignToP),
    ...
    app.get(/^\/[0-9][0-9A-Za-z]+(\/.*)?/, fetchIndexForConversation);
    app.get(/^\/explore\/[0-9][0-9A-Za-z]+(\/.*)?/, fetchIndexForConversation);
    app.get(/^\/share\/[0-9][0-9A-Za-z]+(\/.*)?/, fetchIndexForConversation);
    app.get(/^\/summary\/[0-9][0-9A-Za-z]+(\/.*)?/, fetchIndexForConversation);
    app.get(/^\/ot\/[0-9][0-9A-Za-z]+(\/.*)?/, fetchIndexForConversation);
